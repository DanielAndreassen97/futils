...
func Execute(client FabricClient, token string, target fabric.Workspace, plan []PlannedItem, rb *Rebinder, seed map[string]string, modelsByWS map[string]map[string]string, done *int64) ([]Result, []PendingReportRebind, error) {
	...
	for _, p := range plan {
		func() {
			...
			def, parts, err := buildDefinition(p.Item, target.ID, idMap, resolver, rb)
			if err != nil {
				res.Err = err
...
func buildDefinition(item LocalItem, targetWS string, idMap map[string]string, resolver *Resolver, rb *Rebinder) (*fabric.Definition, map[string][]byte, error) {
	parts, _, err := SubstituteParts(item, idMap, resolver, rb, targetWS)
	if err != nil {
		return nil, nil, err
	}
	if len(item.Parts) == 0 {
