...
- **Deploy from git** — deploy a Fabric git repo to target workspaces, straight from origin (never your working tree). The branch is auto-detected from `origin/HEAD` (falling back to `main`, then `master`), or pinned per customer — deploy from `origin/dev`, a release branch, whatever fits your flow. Self-contained: no `parameter.yml` or deployment pipelines needed.
  ...
  - **Same-workspace pipeline references** — Fabric's git format writes `00000000-0000-0000-0000-000000000000` as the `workspaceId` of a reference to an item in the pipeline's own workspace, and its git-sync fills in the syncing workspace on the way in. The REST items API does not, so futils substitutes the target workspace itself — a deployed pipeline points at TEST's own items instead of carrying the placeholder into TEST.
...
- **Manage workspaces** — the master screen for working inside a workspace, sitting alongside the per-customer Run/Refresh/Move entries rather than replacing them. Create, rename and delete Fabric workspaces without opening the portal.
  - **Everything in the workspace, on one screen** — opening a workspace lists every item it holds, grouped by type and coloured the way Fabric colours them (lakehouse blue, notebook and pipeline green, semantic model purple, report gold), with the workspace's own actions pinned above. Type to filter the items; the workspace actions stay put so Rename is always one arrow away.
  - **Act on an item where you found it** — run a notebook, run a pipeline, refresh a semantic model's tables, or move a Report/SemanticModel/Notebook to another workspace. These call the same flows as the top-level entries with the workspace and item already chosen, so collision handling, parameter forms and lakehouse repair behave identically either way.
  - **Item metadata** — rename an item or edit its description. Deleting a data-bearing item (Lakehouse, Warehouse, …) needs the word `Yes` typed; other types take a plain confirm, since their content lives in git. Config entries naming the item — favourites, post-deploy runs, reference overrides, substitutions — are listed but never rewritten automatically: the same item name resolves in every environment, so futils asks instead of guessing. Pick a workspace from a type-to-filter list and see its capacity, item counts per type, your role, and which futils config entries point at it. Creating offers a capacity picker and then registers the new workspace in one of the customer's environments. Deleting takes every item with it, so it prints that blast radius and only proceeds once you type `Yes`. Because futils stores workspaces by *name*, a rename or delete also repairs every config reference — environment workspace lists, deploy mappings and per-mapping baselines — so a rename can't silently break a deploy. Rename and delete need the Admin workspace role, which futils checks up front instead of letting Fabric answer `403`.
